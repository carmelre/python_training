...
from python_training.document_gen import collect_filenames
import os
...
def test_filename_collection():
    filename_gen_result = list(collect_filenames(FULL_TEST_DIR))
...
def test_indexed_data_validation(elasticsearch_test):
    index_directory(FULL_TEST_DIR, elasticsearch_test, TEST_INDEX, TEST_DOCTYPE, FILE_METADATA_DEFAULT_MAPPING)
    original_docs = generate_es_metadata_docs(FULL_TEST_DIR, TEST_DOCTYPE)
